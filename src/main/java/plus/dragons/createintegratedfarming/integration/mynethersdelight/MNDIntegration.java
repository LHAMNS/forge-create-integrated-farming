...
/**
 * Entry point for the My Nether's Delight integration module.
 * All calls to this class are guarded behind {@link ModIntegration#MY_NETHERS_DELIGHT}{@code .enabled()}
 * checks in {@link CIFCommon}, so MND classes are never loaded unless the mod is present.
 */
public class MNDIntegration {
    /**
     * Called during mod construction if My Nether's Delight is loaded.
     * Registers deferred registries onto the mod event bus.
     */
    public static void registerCommon(IEventBus modBus) {
        MNDArmInteractionPointTypes.register(modBus);
    }

    /**
     * Called during {@link net.minecraftforge.fml.event.lifecycle.FMLCommonSetupEvent}
     * if My Nether's Delight is loaded. Registers behaviours that require
    ...
    /**
     * Called during client construction if My Nether's Delight is loaded.
